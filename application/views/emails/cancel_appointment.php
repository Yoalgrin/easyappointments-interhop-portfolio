...
<div id="header" style="background-color: #1F1BDD; height: 45px; padding: 10px 15px;">
    ...
    <div id="content" style="padding: 10px 15px;">
        ...
        <!--Recap mail for providers & secretaries !-->
        <?php
            if(gettype($appointment_service) != 'string'):
        ?>

        <p><?= lang('appointment_cancelled_for_provider') ?></p>

                <?php
                $i = 0;
                while($i < sizeof($appointment_service)):
                    $aptService = $appointment_service[$i];
                    $aptDate = $appointment_date[$i];
                    $aptDuration = $appointment_duration[$i];
                    $customerName = $customer_name[$i];
                    $customerEmail = $customer_email[$i];
                    $customerPhone = $customer_phone[$i];
                    $customerAdress = $customer_address[$i];
        ?>

        <h2><?= lang('appointment_details_title') . ' '. ($i+1) ?></h2>
        <table id="appointment-details">
            <tr>
                <td class="label" style="padding: 3px;font-weight: bold;"><?= lang('service') ?></td>
                <td style="padding: 3px;"><?= $aptService?></td>
            </tr>
            <tr>
            ...
            <tr>
                ...
                <td style="padding: 3px;"><?= $aptDate ?></td>
            </tr>
            <tr>
                <td class="label" style="padding: 3px;font-weight: bold;"><?= lang('duration') ?></td>
                <td style="padding: 3px;"><?= $aptDuration ?></td>
            </tr>
            <tr>
        ...
        ?>
        <!--Generic mail for customers !-->
        <p><?= lang('appointment_cancelled_by_provider') ?></p>
